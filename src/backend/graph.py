...
def call_llm(state: State):
  ...
  system_prompt = (
    ...
    "\n6. CRITICAL: MAX 2 tool calls at once."
    "\n7. NO PARALLEL EXECUTION: Do NOT call a 'read' tool (like get_calendar_events) and a 'write' tool (like add_calendar_events) at the same time in a single turn."
    "\n8. Step-by-step sequence: If a user asks you to schedule something, first call get_calendar_events to check for free time."
    "\n9. Wait for the system to return the calendar data to you. ONLY AFTER you have read the data and confirmed there are no conflicts, you may call add_calendar_events."
    "\n10. Before executing tool calls, privately list out every specific quantitative constraint requested by the user to ensure you do not miss any."
    "\n\n"
    "FORMATTING RULES:"
